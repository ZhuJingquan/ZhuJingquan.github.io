...
class ApolloApp {
 public:
   ...
   */
  virtual std::string Name() const = 0;//确定节点名称
 ...
 protected:
   ...
   * register a timer callback function.//此处介绍了节点的调用方式：
...
#define APOLLO_MAIN(APP)     //定义宏函数，Effective C++ 中推荐用模板类的方式                               \
  int main(int argc, char **argv) {                            \
    google::InitGoogleLogging(argv[0]);                        \
    google::ParseCommandLineFlags(&argc, &argv, true);         \
    signal(SIGINT, apollo::common::apollo_app_sigint_handler); 
//SiGINT 系统定义的信号宏,只是定义系统中断后的处理\
...
[C++ signal 函数使用](http://blog.csdn.net/ta893115871/article/details/7475095)
